...
class UNet_3Plus_DeepSup_CGM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # added output to train cls layers
        cls_output = self.cls(hd5).squeeze(3).squeeze(2)

        return F.sigmoid(d1), F.sigmoid(d2), F.sigmoid(d3), F.sigmoid(d4), F.sigmoid(d5), cls_output
